  ...
  <div>
    <div v-if="results" class="grid lg:grid-cols-3 gap-5">
      ...
      <div class="lg:col-span-2 -mt-5">
        <Level class="lg:ml-10" title="Alif - Basic Level" />
        <ol
        ...
        <div id="exam" class="pl-10">
          ...
          >
            ...
            <div class="flex items-center">
              <BootstrapIcon
                v-if="grade >= 60 && last_lesson > 29"
                class="text-2xl text-green-700"
                name="shield-check"
              />
              <BootstrapIcon
                v-if="last_lesson < 29 || !authenticated"
                class="text-2xl"
                name="lock-fill"
              />
            </div>
...
const results = ref();
try {
  results.value = await $fetch(BASEAPIURL.value + "/api/lessons/basic-level");
} catch (error) {
  // console.log(error);
}
...
const exam = async () => {
  ...
  router.push("/dashboard/exam/basic-level-question-1");
};
